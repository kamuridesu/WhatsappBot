...
if [ $(echo $PREFIX | grep -o "com.termux") ]; then # Termux (Android)
    ...
    apt update 2>&1 > /dev/null && curl -fsSL https://deb.nodesource.com/setup_16.x | sudo -E bash && sudo apt-get install nodejs && apt install ffmpeg git webp -y || pacman -Syu 2>&1 >/dev/null && pacman -S nodejs ffmpeg libwebp git
fi
