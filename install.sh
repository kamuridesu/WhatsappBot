...
if [ $(echo $PREFIX | grep -o "com.termux") ]; then # Termux (Android)
    pkg update
    pkg install curl nodejs ffmpeg libwebp git -y  # Instalando pacotes necessários para o funcionamento do script (NodeJS, FFmpeg, LibWebP, Git) e silenciando os outputs.
    echo "alias node='/data/data/com.termux/fiels/usr/bin/node'" >> /data/data/com.termux/files/usr/etc/bash.bashrc
